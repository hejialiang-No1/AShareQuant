...
# 清理旧的产物：用 move-aside 而非 rm -rf，避免触碰批量删除安全策略
if [ -d "$BUILD/$APP_NAME.app" ]; then
  mv "$BUILD/$APP_NAME.app" "/tmp/old-${APP_NAME}-$(date +%s).app" 2>/dev/null || true
fi
mkdir -p "$BUILD"
...
$PB -c "Set :CFBundleIdentifier com.asharequant.terminal" "$PLIST"
...
[ -f "$DMG" ] && mv "$DMG" "/tmp/old-${APP_NAME}-$(date +%s).dmg" 2>/dev/null || true
echo "==> 生成 dmg"
