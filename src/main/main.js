  ...
  const template = [
    {
      label: 'AShareQuant',
      submenu: [
        { role: 'about', label: '关于 AShareQuant' },
        { type: 'separator' },
        { role: 'hide', label: '隐藏' },
...
async function runScan(options = {}) {
  ...
  try {
    ...
    // 1.5) 先批量取一次实时行情：因子里的涨停强度、换手率都依赖它
    let quoteMap = new Map();
    try {
      const qt = await ds.quotes(targets.map((t) => t.secid), { ttl: 30000 });
      quoteMap = new Map(qt.map((q) => [q.secid, q]));
    } catch {
      /* 行情失败则因子退化为纯 K 线口径 */
    }
      ...
      while (cursor < targets.length) {
        ...
        try {
          ...
          if (k && k.bars && k.bars.length >= 60) {
            const a = Factors.analyze(k.bars, quoteMap.get(t.secid) || null);
            if (a) {
              results.push({
                secid: t.secid,
                code: t.code || k.code,
...
function registerIpc() {
  ...
  ipcMain.handle('ds:sectors', (_, type) => ds.sectors(type || 'hy'));
  ...
  ipcMain.on('notify', (_, { title, body }) => {
    if (Notification.isSupported()) {
      const n = new Notification({ title: title || 'AShareQuant', body: body || '', silent: false });
      n.show();
    }
  });
